...
- **機能**:
  - `RAMDISK_PATH` (例: `/mnt/temp/hoshikage`) に `tmpfs` を `mount` コマンドでマウントします（要sudo権限）。
  - 指定サイズ (`RAMDISK_SIZE`) の領域を確保し、モデルファイルをコピーしてからロードします。
- **効果**: SSD読み込みを回避し、ロード時間を50%以上短縮。
- **自動アンマウント**: 長時間（`GREAT_TIMEOUT` デフォルト60分）完全非アクティブな場合、RAMディスクを `umount` し、システムメモリを完全に返却します。

### 📝 FT-008: ログファイル管理
**優先度: 低**
標準出力だけでなく、ファイルへのログ保存をサポートします。
- **機能**: 環境変数 `LOG_FILE_PATH` で指定された場合、そのファイルにログを追記します。
- **ローテーション**: ファイルサイズや日付に基づいてログをローテーションし、ディスク容量の圧迫を防ぎます。
