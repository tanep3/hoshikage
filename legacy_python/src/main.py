...
# Flash AttentionとKV Cacheの有効化設定
os.environ["LLAMA_FLASH_ATTENTION"] = "1"
# KV Cacheをf16に量子化
os.environ["LLAMA_F4_KV"] = "1"
# n_gpu_layersを動的に設定（これだけがサポートされている）
os.environ["LLAMA_N_GPU_LAYERS"] = str(N_GPU_LAYERS)
        ...
        async with self.llm_lock:
            ...
            try:
                ...
                if ram_model_path is None:
                    raise FileNotFoundError(f"モデル '{self.current_model}' が見つかりません")
                
                # 注: llama-cpp-pythonのLlamaクラスはFlash AttentionやKV Cache量子化を
                # 直接サポートしていません。これらを有効化するには：
                # 1. llama-cppを特殊なコンパイルオプションでビルドする
                # 2. 環境変数を設定する
                # 3. llama-cppの低レベルAPIを使用する（CGOバインディング）
                ...
                self.llm = Llama(
                    ...
                    verbose=False,    # 出力を抑制
                    # Flash AttentionとKV Cacheの有効化はllama-cppのコンパイル時設定が必要
                    # 詳細: https://github.com/ggerganov/llama.cpp/blob/master/README.md#flash-attention
                )
